...
| ID | Date | Type | Change |
|---|---|---|---|
| U-004 | 2026-07-04 | Update | Review feedback (robustness QoL): chain a pre-existing `textPreprocessor` instead of clobbering it (restore on dispose); guard PlayerLoop install against a duplicate tick; warn (all builds, once per span build) on an unresolved effect key with a cross-channel hint; add a `TextEffect.ValidateArgs` hook (with `EffectArgs.IsFloat`/`IsInt` predicates) so a bad inline arg like `wave(foo)` warns at build time; make registry key lookups case-insensitive to match the preprocessor's tag/attribute matching. |
...
- **Robustness QoL (U-004):** five author-facing hardening changes. (1) *Preprocessor chaining:* `AnimatedText` captures whatever `textPreprocessor` the label already had, passes it to `AnimTagPreprocessor` as an inner that runs first (its substitutions land before `<anim>` stripping, so span indices still map into the final string), and restores it on `Dispose` instead of nulling. A leftover `AnimTagPreprocessor` is never chained, else it would strip our tags before we see them. (2) *Idempotent install:* `TextAnimator.Install` depth-first scans the player loop for an existing `typeof(TextAnimator)` system and returns early, so a second `RuntimeInitializeOnLoadMethod` pass (domain reload disabled) cannot stack a duplicate tick. (3) *Unresolved-key diagnostics:* `FlushRun` warns when a specified key resolves to nothing, peeking the opposite channel so `motion="rainbow"` says "did you mean color=". (4) *Arg validation:* new `virtual TextEffect.ValidateArgs(in EffectArgs)` (default null) called once per span build; built-ins override via the `ValidateFloats(args, ...names)` helper, backed by `EffectArgs.IsFloat`/`IsInt` (true when a slot is absent or parses, false only when present-but-garbage). Parsing stays per-frame and non-allocating; only the warning moved to build time. (5) *Case-insensitive keys:* both registry dictionaries use `StringComparer.OrdinalIgnoreCase`, matching the preprocessor's already case-insensitive tag/attribute matching, which also makes duplicate-key detection case-insensitive. Diagnostics warn in all builds (not gated to editor/dev) by choice; they fire at span-build time, not per frame, so a shipped game logs at most once per bad tag per text change. Rejected: gating warnings behind `#if DEBUG` (author feedback wanted in built jam games too); caching parsed args to also kill the per-frame parse (deferred, the parse is non-allocating and cheap).
